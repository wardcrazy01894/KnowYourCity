...
- [x] **St. Pete dataset** — inclusive food/drink via `fetch-food` + curated
      ...
      truly-closed, and added 4 new spots → 396; then removing 13 national chains
      that leaked past the original fame pass → **383** (all in play — cap is 400).
...
- [x] **Play cap per city (`City.playCap`) + non-food floor.** Every enriched
      ...
      medium / 20% hard). Caps: St. Pete 400 (383 rows, all in play), Ann Arbor
...
- [x] **Difficulty system (St. Pete)** — per-location easy/medium/hard from a
      ...
      rename 4 to successors, −5 closed, +4 new; **383** after removing 13
      national chains). PR #40. (Rollout to other
...
- [x] Data pipeline (M2): Overpass scripts (fetch-pois / fetch-food /
      build-city) → per-city `public/locations.<id>.json` (St. Pete 383 +
