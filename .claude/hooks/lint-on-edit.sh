...
# (this is what kills the trailing "style: prettier" fixup commits, and keeps CI
# green so PRs aren't blocked by the required `build / typecheck / lint` check).
#
...
# Only act on TypeScript/TSX files that actually exist.
case "$file" in
  *.ts | *.tsx) ;;
  *) exit 0 ;;
esac
...
cd "$root" || exit 0
# Need a local install for the linters; if absent, don't block editing.
