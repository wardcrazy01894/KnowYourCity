...
### Status: 379 locations (after the fame pass cleanup + parks/lakes + play-cap re-run)
`public/locations.stpete.json` holds **379 St. Pete places** (peaked at 401 after
...
−17 national chains that had leaked past the original fame pass (Bob Evans,
...
filters to `inPlay !== false`. Current caps: St. Pete 400 (379 rows, all in
...
§4c): St. Pete (379 → **379**), State College (230 → **200**), Ann Arbor (341 →
